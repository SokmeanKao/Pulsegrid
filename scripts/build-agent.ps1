...
try {
  ...
  $env:GOOS = "windows"
  $env:GOARCH = "amd64"
  go build -ldflags "-s -w" -o (Join-Path $OutDir "pulsegrid-agent.exe") ./cmd/pulsegrid-agent

  Write-Host "Building Linux amd64 → pulsegrid-agent-linux-amd64"
  $env:GOOS = "linux"
  $env:GOARCH = "amd64"
  go build -ldflags "-s -w" -o (Join-Path $OutDir "pulsegrid-agent-linux-amd64") ./cmd/pulsegrid-agent
